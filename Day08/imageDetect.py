...
def chinese_plot_box(image,label,x,sizes,fontcolor=(255,0,0),line_thickness=20):
    cv2img = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    pilimg = Image.fromarray(cv2img)
    draw = ImageDraw.Draw(pilimg)
    font = ImageFont.truetype("./5c78c0705c579.ttf",50,encoding = "GB2312")
...
def detect():
    ...
    if result != None:
         ...
         for locationinfo in result:
             ...
             img = chinese_plot_box(img,label=name,x=[left+20,top+20,height,width],sizes=20,fontcolor=(0,255,255))
